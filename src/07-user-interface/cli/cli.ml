...
type config = { filenames : string list; use_stdlib : bool; compile : bool; time : bool; benchmark : bool }

let parse_args_to_config () =
  let filenames = ref [] and use_stdlib = ref true  and compile = ref false and time = ref false and benchmark = ref false in
  ...
  { filenames = List.rev !filenames; use_stdlib = !use_stdlib; compile = !compile; time = !time; benchmark = !benchmark }
...
let main () =
  ...
  try
    ...
    let final_state = 
      ...
      | true -> 
      let state = Loader.load_source Loader.initial_state Loader.stdlib_source in
    ...
    tf (fun () ->
      ...
      | true ->
        let out_str = Compiler.compile_prog final_state.compiler final_state.typechecker.type_definitions in
