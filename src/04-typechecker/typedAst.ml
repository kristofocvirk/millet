...
let make_const_expr const : expression = 
  match const with
  | Ast.Const (Const.String s) -> Source.{it = Const (Const.String s); et = Some (Ast.TyConst Const.StringTy)}
  | Ast.Const (Const.Float f) -> Source.{it = Const (Const.Float f); et = Some (Ast.TyConst Const.FloatTy)}
  | Ast.Const (Const.Integer i) ->Source.{it = Const (Const.Integer i); et = Some (Ast.TyConst Const.IntegerTy)}
  | Ast.Const (Const.Boolean b) ->Source.{it = Const (Const.Boolean b); et = Some (Ast.TyConst Const.BooleanTy)}
  | _ -> failwith "this function only takes constant expressions"

let make_const_pat const : pattern = 
  match const with
  | Ast.PConst (Const.String s) -> Source.{it = PConst (Const.String s); et = Some (Ast.TyConst Const.StringTy)}
  | Ast.PConst (Const.Float f) -> Source.{it = PConst (Const.Float f); et = Some (Ast.TyConst Const.FloatTy)}
  | Ast.PConst (Const.Integer i) ->Source.{it = PConst (Const.Integer i); et = Some (Ast.TyConst Const.IntegerTy)}
